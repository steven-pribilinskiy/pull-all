...
export const keymap: KeymapSection[] = [
  {
    ...
    bindings: [
      ...
      { keys: ['t'], action: 'Column-toggle leader', note: 'then u/a/d/l/w/b/s → status/ahead-behind/dirty/last/worktrees/branches/stashes; stays active until Esc. The status column shows a short label with the failure kind (not found / auth / diverged / timeout / network / lock) on failed repos. Zeros render dim; a column every repo lacks (no worktrees/stashes, ≤1 branch) auto-hides and its menu chip is dim/inert', keywords: ['columns', 'status', 'reason', 'error'] },
